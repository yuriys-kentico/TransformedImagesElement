export namespace NumberUtils {
    export const isDigitsOptionallyDotAndDecimals = (value: string, decimals: number) => new RegExp(`^\\d*\\.?\\d{0,${decimals}}$`).test(value);
    ...
    export const ensureBetween = (value: string, max: number, min: number): string => {
        const tempValue = NumberUtils.toNumber(value);

        if (tempValue !== NaN && (tempValue > max || tempValue < min)) {
            return NumberUtils.toBetween(tempValue, max, min).toString();
        }
